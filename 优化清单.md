# KaizenShowcase 优化清单

> 项目当前完成度：**85%**
> 文档生成日期：2024年12月24日

---

## 项目概况

| 指标 | 状态 |
|------|------|
| 核心功能 | ✅ 已完成 |
| 代码质量 | ⚠️ 需优化 |
| 安全性 | ❌ 有问题 |
| 测试覆盖 | ❌ 无测试 |
| 用户体验 | ⚠️ 可提升 |
| 生产就绪 | ❌ 未就绪 |

---

## 优化清单

### P0 - 必须修复（安全/稳定性问题）

#### 1. ❌ 安全漏洞：webSecurity 关闭

**位置**: `electron/main.js:19`

```javascript
// 当前代码
webPreferences: {
  webSecurity: false  // ⚠️ 严重安全隐患
}
```

**问题**:
- 关闭 webSecurity 允许任意跨域请求
- 存在本地文件泄露风险
- 可能被恶意脚本利用

**修复方案**:
```javascript
webPreferences: {
  webSecurity: true,  // 保持开启
  contextIsolation: true,
  nodeIntegration: false,
  preload: path.join(__dirname, 'preload.js')
}
```
配合正确配置的 `local-video://` 协议即可加载本地视频，无需关闭 webSecurity。

---

#### 2. ❌ 缺少全局错误处理

**问题**: 所有 async 函数都没有 try-catch，应用可能静默失败

**影响文件**:
- `src/components/ProjectList.jsx` - loadProjects, handleCreateProject, handleDeleteProject
- `src/components/StageManager.jsx` - loadStages, handleCreateStage, handleDeleteStage
- `src/components/ProcessList.jsx` - handleCreateProcess, handleUpdateProcess, handleDeleteProcess
- `src/components/ComparePlayer.jsx` - handlePlay, playNextProcess, playPrevProcess
- `src/components/VideoPlayer.jsx` - handlePlay
- `src/App.jsx` - loadProcesses

**修复方案**:

1. 添加 React Error Boundary:
```jsx
// src/components/ErrorBoundary.jsx
class ErrorBoundary extends React.Component {
  state = { hasError: false, error: null };

  static getDerivedStateFromError(error) {
    return { hasError: true, error };
  }

  render() {
    if (this.state.hasError) {
      return <div className="error-page">出错了: {this.state.error?.message}</div>;
    }
    return this.props.children;
  }
}
```

2. 为每个 async 函数添加 try-catch:
```javascript
const loadProjects = async () => {
  try {
    const allProjects = await window.electronAPI.getAllProjects();
    setProjects(allProjects);
  } catch (error) {
    console.error('加载项目失败:', error);
    // 显示用户友好的错误提示
  }
};
```

---

### P1 - 高优先级（影响用户体验）

#### 3. ⚠️ 使用原生 confirm 对话框

**位置**:
- `src/components/ProjectList.jsx:39`
- `src/components/StageManager.jsx:42`
- `src/components/ProcessList.jsx:74`

```javascript
// 当前代码
if (confirm('确定要删除这个项目吗？'))
```

**问题**:
- 样式与应用不统一
- 无法自定义按钮文字
- 用户体验差

**修复方案**: 创建自定义确认对话框组件
```jsx
// src/components/ConfirmDialog.jsx
function ConfirmDialog({ isOpen, title, message, onConfirm, onCancel }) {
  if (!isOpen) return null;
  return (
    <div className="modal-overlay">
      <div className="modal confirm-dialog">
        <h3>{title}</h3>
        <p>{message}</p>
        <div className="modal-actions">
          <button className="btn-secondary" onClick={onCancel}>取消</button>
          <button className="btn-danger" onClick={onConfirm}>确认删除</button>
        </div>
      </div>
    </div>
  );
}
```

---

#### 4. ⚠️ 工具函数重复定义

**问题**: `formatTime` 和 `formatTimeSaved` 函数在多个文件中重复

**重复位置**:
- `src/components/ProcessList.jsx:18-33`
- `src/components/VideoPlayer.jsx:84-99`
- `src/components/ComparePlayer.jsx:270-285`

**修复方案**: 创建工具函数库
```javascript
// src/utils/time.js
export const formatTime = (seconds) => {
  const mins = Math.floor(seconds / 60);
  const secs = Math.floor(seconds % 60);
  return `${mins.toString().padStart(2, '0')}:${secs.toString().padStart(2, '0')}`;
};

export const formatTimeDetailed = (seconds) => {
  const mins = Math.floor(seconds / 60);
  const secs = (seconds % 60).toFixed(1);
  return mins > 0 ? `${mins}分${secs}秒` : `${secs}秒`;
};

export const formatTimeSaved = (timeSaved) => {
  if (!timeSaved || timeSaved === 0) return '无变化';
  const absTime = Math.abs(timeSaved);
  const timeStr = formatTime(absTime);
  return timeSaved > 0 ? `节省 ${timeStr}` : `增加 ${timeStr}`;
};
```

---

#### 5. ⚠️ 缺少 Loading 状态

**问题**: 数据加载时用户无感知，体验差

**需要添加 Loading 的位置**:
- 项目列表加载
- 阶段列表加载
- 工序列表加载
- 视频加载

**修复方案**:
```jsx
const [isLoading, setIsLoading] = useState(false);

const loadProjects = async () => {
  setIsLoading(true);
  try {
    const allProjects = await window.electronAPI.getAllProjects();
    setProjects(allProjects);
  } finally {
    setIsLoading(false);
  }
};

// 渲染
{isLoading ? <LoadingSpinner /> : <ProjectGrid projects={projects} />}
```

---

#### 6. ⚠️ 缺少类型检查

**问题**: 没有 PropTypes 或 TypeScript，容易产生运行时错误

**修复方案A - 添加 PropTypes（快速）**:
```jsx
import PropTypes from 'prop-types';

ProjectList.propTypes = {
  onProjectSelect: PropTypes.func.isRequired
};

ComparePlayer.propTypes = {
  process: PropTypes.object,
  processes: PropTypes.array,
  stage: PropTypes.object.isRequired,
  layoutMode: PropTypes.oneOf(['horizontal', 'vertical']),
  globalMode: PropTypes.bool,
  onProcessChange: PropTypes.func
};
```

**修复方案B - 迁移 TypeScript（推荐）**:
```typescript
interface Process {
  id: number;
  name: string;
  before_start_time: number;
  before_end_time: number;
  after_start_time: number;
  after_end_time: number;
  time_saved: number;
  process_type: 'normal' | 'new_step' | 'cancelled';
}

interface ComparePlayerProps {
  process?: Process;
  processes?: Process[];
  stage: Stage;
  layoutMode: 'horizontal' | 'vertical';
  globalMode?: boolean;
  onProcessChange?: (process: Process) => void;
}
```

---

### P2 - 中优先级（功能增强）

#### 7. 🔧 缺少单元测试

**当前状态**: 无任何测试文件

**建议添加**:
- 单元测试: Jest + React Testing Library
- E2E测试: Playwright 或 Cypress
- 目标覆盖率: 70%+

**测试文件结构**:
```
src/
├── components/
│   ├── __tests__/
│   │   ├── ProjectList.test.jsx
│   │   ├── StageManager.test.jsx
│   │   ├── ProcessList.test.jsx
│   │   ├── VideoPlayer.test.jsx
│   │   └── ComparePlayer.test.jsx
│   └── ...
├── utils/
│   └── __tests__/
│       └── time.test.js
└── ...
```

---

#### 8. 🔧 状态管理需要优化

**当前问题**:
- 所有状态集中在 `App.jsx`
- Props 层层传递（props drilling）
- 难以扩展

**修复方案 - 使用 React Context**:
```jsx
// src/context/AppContext.jsx
const AppContext = createContext();

export function AppProvider({ children }) {
  const [currentProject, setCurrentProject] = useState(null);
  const [currentStage, setCurrentStage] = useState(null);
  const [processes, setProcesses] = useState([]);

  return (
    <AppContext.Provider value={{
      currentProject, setCurrentProject,
      currentStage, setCurrentStage,
      processes, setProcesses
    }}>
      {children}
    </AppContext.Provider>
  );
}

export const useApp = () => useContext(AppContext);
```

---

#### 9. 🔧 工序无法拖拽排序

**当前状态**: 只能通过数据库 sort_order 字段排序，前端无法交互

**修复方案**: 使用 `@dnd-kit/core` 或 `react-beautiful-dnd`

```jsx
import { DndContext, closestCenter } from '@dnd-kit/core';
import { SortableContext, verticalListSortingStrategy } from '@dnd-kit/sortable';

function ProcessList({ processes, onReorder }) {
  const handleDragEnd = (event) => {
    const { active, over } = event;
    if (active.id !== over.id) {
      onReorder(active.id, over.id);
    }
  };

  return (
    <DndContext collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
      <SortableContext items={processes} strategy={verticalListSortingStrategy}>
        {processes.map(process => (
          <SortableItem key={process.id} process={process} />
        ))}
      </SortableContext>
    </DndContext>
  );
}
```

---

#### 10. 🔧 缺少键盘快捷键

**建议快捷键**:
| 快捷键 | 功能 |
|--------|------|
| `Space` | 播放/暂停 |
| `←` / `→` | 上一个/下一个工序 |
| `Ctrl+N` | 新建工序 |
| `Ctrl+E` | 编辑当前工序 |
| `Delete` | 删除当前工序 |
| `1-5` | 设置播放速度 |

**实现方案**:
```jsx
useEffect(() => {
  const handleKeyDown = (e) => {
    if (e.code === 'Space') {
      e.preventDefault();
      togglePlay();
    }
    if (e.code === 'ArrowLeft') navigatePrev();
    if (e.code === 'ArrowRight') navigateNext();
  };

  window.addEventListener('keydown', handleKeyDown);
  return () => window.removeEventListener('keydown', handleKeyDown);
}, []);
```

---

#### 11. 🔧 缺少数据导出功能

**建议支持格式**:
- PDF 报告（包含统计图表和视频截图）
- Excel 数据表（工序时间明细）
- JSON 备份文件

**依赖库**:
- PDF: `jspdf` + `html2canvas`
- Excel: `xlsx`

---

#### 12. 🔧 视频播放优化

**当前问题**:
- 切换工序时有加载延迟
- 两个视频同步播放可能有偏差

**优化方案**:
1. 预加载下一个工序的视频时间点
2. 使用 `requestAnimationFrame` 精确同步
3. 添加视频缓冲进度显示

```javascript
// 预加载优化
const preloadNextSegment = (video, nextProcess) => {
  video.currentTime = nextProcess.start_time;
  video.preload = 'auto';
};

// 同步校正
const syncVideos = () => {
  const drift = Math.abs(beforeVideo.currentTime - afterVideo.currentTime);
  if (drift > 0.1) {  // 偏差超过100ms
    afterVideo.currentTime = beforeVideo.currentTime;
  }
};
```

---

### P3 - 低优先级（改进项）

#### 13. 📝 样式文件需要模块化

**当前问题**: 所有样式在 `App.css` 一个文件中（700+行）

**修复方案**: 使用 CSS Modules
```
src/
├── components/
│   ├── ProjectList/
│   │   ├── index.jsx
│   │   └── styles.module.css
│   ├── VideoPlayer/
│   │   ├── index.jsx
│   │   └── styles.module.css
│   └── ...
└── styles/
    └── global.css
```

---

#### 14. 📝 缺少应用打包配置

**当前状态**: 无法生成可分发的安装包

**需要添加**: `electron-builder` 配置

```json
// package.json 添加
{
  "build": {
    "appId": "com.kaizen.showcase",
    "productName": "改善效果展示系统",
    "directories": {
      "output": "release"
    },
    "mac": {
      "category": "public.app-category.productivity",
      "target": ["dmg", "zip"]
    },
    "win": {
      "target": ["nsis", "portable"]
    }
  }
}
```

---

#### 15. 📝 缺少代码规范配置

**需要添加**:
- ESLint 配置
- Prettier 配置
- Husky + lint-staged（提交前检查）

```json
// .eslintrc.json
{
  "extends": ["react-app"],
  "rules": {
    "no-unused-vars": "warn",
    "react-hooks/exhaustive-deps": "warn"
  }
}
```

---

#### 16. 📝 其他改进项

| 项目 | 说明 |
|------|------|
| 暗黑模式 | 添加主题切换功能 |
| 国际化 | 使用 react-i18next 支持多语言 |
| 新手引导 | 使用 react-joyride 添加使用指引 |
| 数据备份 | 自动备份数据库到本地 |
| 撤销/重做 | 实现操作历史记录 |
| 卡拉OK式字幕 | 实现高精度语音、字符同步高亮效果 |

---

## 实施建议

### 第一阶段（1周）- 修复关键问题 ✅ 已完成
- [x] P0-1: 修复 webSecurity 安全问题 ✅
- [x] P0-2: 添加全局错误处理 ✅
- [x] P1-3: 替换原生 confirm 对话框 ✅
- [x] P1-4: 提取公共工具函数 ✅
- [x] 添加 Toast 通知组件 ✅
- [x] 为所有组件添加 try-catch 错误处理 ✅

### 第二阶段（1-2周）- 提升体验 ✅ 已完成
- [x] P1-5: 添加 Loading 状态 ✅
- [x] P1-6: 添加 PropTypes 类型定义 ✅
- [x] P1-7: 性能优化 (React.memo) ✅
- [x] P2-9: 实现工序拖拽排序 ✅
- [x] P2-10: 添加键盘快捷键 ✅
- [x] P2-11: 添加数据导出功能 ✅

### 第三阶段（2周）- 功能完善
- [ ] P2-7: 添加单元测试
- [ ] P2-8: 优化状态管理 (Context)
- [ ] P2-12: 视频播放优化

### 第四阶段（持续）- 持续改进
- [ ] P3 项目按需实施

---

## 技术债务评估

| 维度 | 当前评分 | 目标评分 | 备注 |
|------|----------|----------|------|
| 代码质量 | 6/10 | 8/10 | 需添加类型检查和测试 |
| 安全性 | 4/10 | 9/10 | 必须修复 webSecurity |
| 性能 | 7/10 | 8/10 | 视频同步需优化 |
| 可维护性 | 6/10 | 8/10 | 需要模块化重构 |
| 用户体验 | 7/10 | 9/10 | 添加快捷键和交互优化 |
| 可扩展性 | 5/10 | 8/10 | 需要状态管理优化 |

**当前综合评分: 5.8/10**
**目标综合评分: 8.3/10**

---

## 附录：文件修改清单

| 优先级 | 问题 | 涉及文件 |
|--------|------|----------|
| P0 | webSecurity | electron/main.js |
| P0 | 错误处理 | 所有组件文件 |
| P1 | confirm对话框 | ProjectList, StageManager, ProcessList |
| P1 | 重复函数 | ProcessList, VideoPlayer, ComparePlayer |
| P1 | Loading状态 | 所有组件文件 |
| P1 | 类型检查 | 所有组件文件 |
| P2 | 状态管理 | App.jsx, 新建 context 文件 |
| P2 | 拖拽排序 | ProcessList.jsx |
| P2 | 快捷键 | VideoPlayer, ComparePlayer |
